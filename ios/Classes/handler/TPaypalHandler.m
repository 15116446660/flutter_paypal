...
- (void)payPalPaymentDidCancel:(PayPalPaymentViewController *)paymentViewController {
    self.payResult(@{@"code": @(-2), @"result": @{@"response_type": @"cancel payment"}});
...
- (void)payPalPaymentViewController:(PayPalPaymentViewController *)paymentViewController didCompletePayment:(PayPalPayment *)completedPayment {
    ...
    if ([completedPayment.confirmation[@"response"][@"state"] isEqualToString:@"approved"]) {
        // 支付成功
        self.payResult(@{@"code": @0, @"result": completedPayment.confirmation});
    }else {
        // 其他异常，未测试
        self.payResult(@{@"code": @(-1), @"result": completedPayment.confirmation});
    }
    // 退出支付
    [_rootVC dismissViewControllerAnimated:YES completion:nil];
    /*
